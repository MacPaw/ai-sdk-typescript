...
This is `@macpaw/ai-sdk` — a universal TypeScript client for AI Gateway.

## Integrating an app with AI Gateway

Install `@macpaw/ai-sdk`. No other AI packages required.
...
Do NOT import from `@ai-sdk/openai` or `ai` directly for provider/generation functions — use `@macpaw/ai-sdk/provider`. Exception: React hooks (`useChat`, `useCompletion`) stay as `ai/react`.
...
- `@macpaw/ai-sdk/core` is internal/advanced API — prefer the main entry point
