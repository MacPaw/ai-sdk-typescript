...
import type { RequestConfig } from '../core/config';
...
describe('createMockTransport', () => {
  ...
  describe('default handlers', () => {
    ...
    it('returns mock ResponseObject for /responses', async () => {
      const response = await transport.request(
        makeConfig(API_PATHS.Responses, { model: 'gpt-4.1-nano', input: 'Hi' }),
      );
      const data = await json(response);
    ...
    it('returns mock image for /images/generations', async () => {
      const response = await transport.request(
        makeConfig(API_PATHS.ImagesGenerations, { prompt: 'cat' }),
      );
      const data = await json(response);
  ...
  describe('onRoute', () => {
    it('overrides default handler for a path', async () => {
      ...
      const response = await transport.request(
        makeConfig(API_PATHS.ChatCompletions, { model: 'custom-model' }),
      );
      const data = await json(response);
    ...
    it('does not affect other routes', async () => {
      transport.onRoute('/chat/completions', () =>
        new Response(JSON.stringify({ custom: true }), { status: 200 }),
      );
