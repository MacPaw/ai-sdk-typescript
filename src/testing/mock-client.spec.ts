...
import type { ChatCompletion, CreateEmbeddingResponse, CreateImageResponse, TranscriptionResponse, TranslationResponse } from '../core/types';
...
describe('createMockAIGatewayClient', () => {
  ...
  describe('responses', () => {
    it('create tracks calls', async () => {
      ...
      const result = await client.responses.create({ model: 'm', input: 'hi' }) as { output: Array<{ content: Array<{ text: string }> }> };
    ...
    it('stream returns mock stream result', async () => {
      ...
      client.responses.stream.mockReturnValue(
        createMockStreamResponseResult('response text'),
      );
  ...
  describe('embeddings', () => {
    it('create tracks calls with fixture', async () => {
      ...
      const result = await client.embeddings.create({ model: 'text-embedding-3-small', input: 'hi' }) as CreateEmbeddingResponse;
  ...
  describe('models', () => {
    it('getInfo tracks calls with fixture', async () => {
      ...
      client.models.getInfo.mockResolvedValue(
        createMockModelInfoResponse({ models: [{ name: 'gpt-4.1-nano' }] }),
      );
  ...
  describe('images', () => {
    it('generate tracks calls with fixture', async () => {
      ...
      client.images.generate.mockResolvedValue(
        createMockImageResponse({ urls: ['https://example.com/img.png'] }),
      );
  ...
  describe('audio', () => {
    it('transcriptions.create with fixture', async () => {
      ...
      const result = await client.audio.transcriptions.create({ file: new Blob(), model: 'whisper-1' }) as TranscriptionResponse;
    ...
    it('translations.create with fixture', async () => {
      ...
      client.audio.translations.create.mockResolvedValue(
        createMockTranslationResponse({ text: 'Translated' }),
      );

      const result = await client.audio.translations.create({ file: new Blob(), model: 'whisper-1' }) as TranslationResponse;
  ...
  describe('mockClear vs mockReset', () => {
    it('mockClear keeps implementation', async () => {
      const client = createMockAIGatewayClient();
      client.chat.completions.create.mockResolvedValue(
        createMockChatCompletion({ content: 'kept' }),
      );
