...
This is the `@macpaw/ai-sdk` SDK — a TypeScript client for AI Gateway (Chat, Embeddings, Images, Audio, Responses).
...
- Use `@macpaw/ai-sdk/provider` instead of direct `@ai-sdk/openai` or `ai` imports for provider/generation functions.
- React hooks (`useChat`, `useCompletion`) stay as `ai/react` — they are NOT re-exported by `@macpaw/ai-sdk/provider`.
