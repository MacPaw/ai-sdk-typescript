...
if (fs.realpathSync(pkgRoot) === fs.realpathSync(projectRoot)) {
  console.log('ℹ  Running inside the @macpaw/ai repo itself — nothing to copy.');
  process.exit(0);
}
...
if (all || tool === 'cursor') {
  ...
  if (!fs.existsSync(src)) {
    results.push('✗ Cursor skill source not found (is @macpaw/ai installed?)');
  } else {
...
function copyMarkdownTemplate(filename, toolName) {
  ...
  if (!fs.existsSync(src)) {
    return `✗ ${filename} template not found (is @macpaw/ai installed?)`;
  }
  ...
  if (fs.existsSync(dest)) {
    ...
    if (existing.includes('@macpaw/ai')) {
